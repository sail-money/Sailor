...
| **Take or manage leverage** (borrow against collateral) | Protocol + asset allowlists, per-borrow cap, LTV ceiling (requires both oracles), position to the SMA | Collateral supply: `sailor-templates` (deposit); borrow: `sailor-templates` (borrow) (Aave variable-rate, Morpho Optimizer, Compound cTokens); unwind (`repay`) is bespoke — no shared template; other markets or cumulative-position bounds → bespoke. Approve coverage per [`sailor-mandates/references/approvals.md`](../../sailor-mandates/references/approvals.md) for the supply leg AND the repay leg — `borrow()` itself needs none | With zero oracles there is NO LTV ceiling — cap-only; an LTV ceiling trusts its oracles and checks at borrow time, not continuously |
...
| **Automate treasury / payment flows** | Recipient allowlist (or one pinned recipient), token allowlist, per-tx cap | `sailor-templates` (transfer) — an allowlisted set, or a one-entry allowlist for a single pinned recipient; native ETH or differing per-recipient caps → bespoke | A recipient allowlist is only as trustworthy as the key that configures it |
...
permissions exist for: `sailor-templates` (approve-batch)
