...
description: Author, deploy, simulate, and register the CCTP bridge permission that moves USDC between the user's chains, with self-recipient, domain-allowlist, and per-tx cap enforced on-chain. Use when the portfolio spans more than one chain and USDC must move across them.
station: mandate
---
...
`sailor-portfolio`, the routing policy).
