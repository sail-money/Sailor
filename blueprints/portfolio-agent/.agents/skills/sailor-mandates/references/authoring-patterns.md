...
- **A spot/oracle-referenced price band must clear the venue's swap fee, not just "some" tolerance.** Any band that computes its floor from a fee-*exclusive* reference price (a raw pool spot price, an oracle's mid) and compares it against a fee-*inclusive* real `amountOutMin` needs its tolerance set above `fee + the agent's own slippage` (+ headroom for price movement) — see `sailor-templates` (swap-no-oracle), the "⚠️ Tolerance vs. pool fee" section for the formula and a field example. A tolerance at or under that sum doesn't tighten the guard — it silently denies every legitimate trade.
...
- **A bespoke exit permission (debt repay, or a venue `WithdrawPermission` does not recognize) needs its cap sized to the position ceiling, not the entry trade — and reconfigurable, same lesson as the tolerance trap above, applied to amount caps.** A position that accumulated over many small entries needs an exit cap sized to what the FULL position could grow to, not what any single entry ever moved. The projection math and the honest default (generous or effectively unbounded, since the real protection is whatever price floor or venue-side accounting the trade already has) live in `sailor-mandate-planner`'s position-exit sizing rule — don't re-derive it here. Write the cap as a `configure()`/setter, not a constructor-only immutable, for the same reason: a misjudged size should be a one-transaction fix, not a redeploy.
