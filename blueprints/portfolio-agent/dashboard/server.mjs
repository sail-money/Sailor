/**
 * Portfolio agent dashboard — a small, local, read-only server.
 *
 ...
 *
 * Run with `pnpm dashboard`. Port is PORTFOLIO_DASHBOARD_PORT (default 4100, outside
...
const port = Number(process.env.PORTFOLIO_DASHBOARD_PORT ?? 4100);
...
// .sail/dashboard.json ({ "title": "..." }) at onboarding; otherwise "Your portfolio".
const title = (() => {
  try {
    const meta = JSON.parse(fs.readFileSync(path.join(sailDir, "dashboard.json"), "utf-8"));
    return typeof meta.title === "string" && meta.title ? meta.title : "Your portfolio";
  } catch {
...
const server = http.createServer((req, res) => {
  ...
  const html = fs.readFileSync(path.join(here, "portfolio.html"), "utf-8");
