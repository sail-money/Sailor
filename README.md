...
| **Skills** | 21 skills under `.agents/skills/`, organized by the five stations: onboarding and diagnostics, strategy definition, mandate construction (one skill per shared permission template, plus the full custom-permission lifecycle), agent construction with a verified code skeleton, and unattended operation through exit. The skills are the harness: any coding agent reads them natively and follows the same verified path in every project. |
