...
export default function RotateSignerModal({
  ...
  async function provisionKey() {
    ...
    if (password.length < MIN_PASSWORD) {
      setError(`Passphrase must be at least ${MIN_PASSWORD} characters.`)
      return
    }
        ...
        {step === 'done' ? (
          ...
          <>
            ...
            <p className={styles.body}>
              ...
              existing one by private key or recovery phrase. Set a passphrase to encrypt it on disk.
            </p>
            <dl className={styles.meta}>
            ...
            {mode !== 'saved' && (
              <label className={form.field}>
                <span className={form.fieldLabel}>Encryption passphrase</span>
                <input
                  className={form.input}
                  type="password"
