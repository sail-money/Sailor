...
`sailor run` executes **execution strategies** (`.sail/strategies/strategies.json`), not a single hard-coded `src/agent.ts`. Each active strategy binds **one executable** — the default `agent` is `src/agent.ts`; a custom strategy loads the path in its `executable` field, typically `src/strategy/<name>.ts` — to an **SMA** and an optional **chains** list — there are no steps and no pipeline. By default `run` executes every active strategy each tick; `sailor run --strategy <name>` runs just one (the lever for per-strategy schedules). Nothing is auto-seeded: a project has **no strategies until one is created**, so `sailor run` with none reports "No active strategies…" — creating one is [`sailor-strategy`](../sailor-strategy/SKILL.md)'s job.
