...
This skill is **THE** entry point for creating or configuring a strategy — whether the user arrived by normal navigation (this is Station 2) or jumped straight in ("create a strategy", "add a strategy", "wire up a new executable"). Station 2 owns both strategy artifacts: `.sail/strategy.md` (the financial intent) and `.sail/strategies/strategies.json` (the execution config that registers the strategy to run). Steps 1–3 and 5–6 settle the **intent** (the three acts below → `.sail/strategy.md`); steps 4, 7–8 generate the **execution config** (`.sail/strategies/strategies.json`) using `sailor strategy create` and `sailor strategy env set`. The model, `.sail/strategies/strategies.json`, and the full `sailor strategy` CLI live in [references/execution-config.md](references/execution-config.md).
...
**When each step fires.** In Station 2, both strategy artifacts are completed before moving to Station 3. First, the intent (steps 1–3, 5–6) is settled and persisted to `.sail/strategy.md`. Then, using the details gathered (name, SMA, executable default `agent`, chains, and env), run `sailor strategy create` and `sailor strategy env set` (steps 4, 7–8) to register the strategy in `.sail/strategies/strategies.json`. Later at Station 4, the executable code itself (`src/strategy/<name>.ts` or `src/agent.ts`) is authored to fulfill the registered strategy. This skill owns both strategy steps — then hands to **Station 3** ([`../sailor-mandate-planner/SKILL.md`](../sailor-mandate-planner/SKILL.md)).
...
Exit verifier: every dimension concrete (including each position-opening action's `exitPath`), user explicitly confirmed, `.sail/strategy.md` written with `"confirmedByUser": true` and `"version": 3`, AND `.sail/strategies/strategies.json` created via `sailor strategy create` (with per-chain env set via `sailor strategy env set` if needed). Next: **Station 3 — [`sailor-mandate-planner`](../sailor-mandate-planner/SKILL.md)**, which routes each action of the spec to a shared template or bespoke authoring.
