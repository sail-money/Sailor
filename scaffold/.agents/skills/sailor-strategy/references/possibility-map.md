...
|---|---|---|---|
| **Accumulate an asset over time** (recurring buys) | Router + token allowlists, per-tx cap, output pinned to the SMA, min-out floor | [`sailor-template-swap`](../../sailor-template-swap/SKILL.md) (oracle-gated band) or [`sailor-template-swap-no-oracle`](../../sailor-template-swap-no-oracle/SKILL.md); approve coverage per `sailor-mandates/references/approvals.md`; cadence is agent-side | A price floor is only as strong as its price source; a thin pool's spot price can be moved within one transaction |
