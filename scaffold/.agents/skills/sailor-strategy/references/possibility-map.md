...
| **Earn on idle capital** (lending / vault deposits) | Vault/market + token allowlists, per-tx cap, position credited to the SMA | [`sailor-template-deposit`](../../sailor-template-deposit/SKILL.md) (ERC-4626 `deposit`/`mint`, Aave v2/v3); exiting the position itself (the vault's own `withdraw`/`redeem`, or Aave's `withdraw`) is bespoke — no shared template calls it, see [`yield.md`](yield.md)'s routing table; `sailor-template-withdraw` only sweeps tokens already in the SMA (it can sweep a held share token, config-only, as a lighter alternative); other market ABIs → bespoke | The bound constrains where and how much — an allowlisted venue's own honesty is a trust assumption the bound cannot vet |
