...
| Cap semantics | ERC-4626 `mint` (entry) and `redeem` (exit) caps are in **shares**, not underlying: effective asset cap = cap × share price — size accordingly (see `sailor-templates` (deposit) and `sailor-templates` (withdraw)). `deposit`/`withdraw` and both Aave legs cap the asset amount directly |
...
| Action | Route |
|---|---|
| Deposits into ERC-4626 vaults (`deposit`/`mint`) or Aave v2/v3 (`deposit`/`supply`) | `sailor-templates` (deposit) |
| Borrows — Aave variable-rate, Morpho Optimizer/Morpho-Aave, Compound cTokens | `sailor-templates` (borrow) — covers the borrow call only; the collateral-supply leg is a deposit action (row above) with its own approve coverage |
| Repay / unwind a borrow position | **Bespoke via `sailor-mandates`** — no shared `RepayPermission` template exists today. `repay` pulls the debt asset from the SMA via allowance, so it needs approve coverage exactly like deposit/swap (see [`sailor-mandates/references/approvals.md`](../../sailor-mandates/references/approvals.md)); do not treat a borrow strategy as complete once `BorrowPermission` alone simulates clean if the strategy has an exit condition |
| Exit/unwind a vault or lending deposit (the vault's own `withdraw`/`redeem`, or Aave's own `withdraw`) | `sailor-templates` (withdraw) — gates ERC-4626 `withdraw`/`redeem` and Aave v2/v3 `withdraw`. `DepositPermission` gates entry selectors only, so entry and exit are two permissions. Proceeds are pinned to the SMA (`receiver`, `owner`, and `to` must all equal the account), so this exits a position — it does not pay anyone out. **On the `redeem` path the cap is denominated in shares**, whose underlying value floats with the share price; size that cap accordingly. The token allowlist binds the Aave path only — an ERC-4626 vault is constrained by the target allowlist alone. Venues whose exits pay `msg.sender` with no calldata recipient (Compound v2/v3, Aave v4) are unrecognized and deny — those remain **bespoke via `sailor-mandates`** |
| Paying proceeds OUT of the SMA to a fixed recipient (e.g. the owner's Safe) | `sailor-templates` (transfer) with a one-entry recipient allowlist — a withdraw exits a position into the SMA, it never sends funds onward. Exiting and paying out are two actions; a strategy that must end with funds at the owner needs both |
| Approve → deposit → reset in one atomic batch | `sailor-templates` (approve-batch) |
| Staking wrapped as an ERC-4626 vault (stake via `deposit`/`mint`) | `sailor-templates` (deposit) — same as any vault; native/LST staking with no vault interface is bespoke via `sailor-mandates` |
| Markets outside those selectors (Morpho Blue, exotic vaults, staking contracts) | bespoke via `sailor-mandates` |
