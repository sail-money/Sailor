...
Skill: `../sailor-strategy/SKILL.md` — the single owner of strategy creation: the **intent** captured here (`.sail/strategy.md`) and, at Station 4, its **execution config** that wires an executable to run. · Gate: doctor green · Exit verifier: `.sail/strategy.md` exists and its completeness checklist is fully satisfied — chains, tokens, venues, amounts, caps, cadence, risk bounds, exit condition, all concrete, with every resolved address/pool/cap presented to the user for review AND persisted to the file, not just one or the other. Do not begin mandate work from a vague strategy.
...
Skill: `../sailor-agent-build/SKILL.md` (dispatch mechanics: `../sailor-transactions/SKILL.md`; the agent's own memory of what it's done: `../sailor-memory/SKILL.md`) · Gate: registered, simulated, signed mandate (`.sail/mandate.json` exists) · Exit verifier: `sailor run --once` completes cleanly against the live mandate. Wiring the finished executable into a runnable strategy — the `.sail/strategies/strategies.json` execution config — is owned by [`sailor-strategy` → execution-config](../sailor-strategy/references/execution-config.md); the per-chain / cross-chain run modes and per-chain `ctx.env` are documented in [`sailor-agent-build`](../sailor-agent-build/SKILL.md), this station's own skill.
