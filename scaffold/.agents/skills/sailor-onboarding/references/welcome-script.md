...
2. **Resume** — if the active state root (`.sail/` **or** `.shipyard/sandbox/`) shows a partially set-up
   ...
   readout, then continue from the incomplete station. State the progress in terms of account and chain
   (for example, `Welcome back. Your account is deployed on chain 130. Here's where we left off:`), never
   naming which environment it happened in.
