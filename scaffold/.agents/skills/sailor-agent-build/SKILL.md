...
**Fund the SMA with trading capital — the step Station 1 deliberately skipped.** Station 1 funded gas (the owner and agent wallets, so they can submit transactions) — never the token(s) the agent actually trades with, because at that point the strategy didn't exist yet. It exists now: read the strategy's `.sail/strategies/<name>.md` resolved `actions[]` and enumerate every distinct `(chain, tokenIn.symbol, tokenIn.address)` pair across them — a per-chain strategy can replay on several chains, and a cross-chain strategy can act on several, so there may be more than one. Tell the user plainly, by name, for EACH one: "Your agent trades from `<SMA address>` on `<chain>` — send it the `<tokenIn.symbol>` you want it to manage" (the SMA is the same address on every supported chain, but only acts on the chains and tokens its actions name — list all of them, not just one). Show the current balance if you can — `sailor ui start` opens the dashboard, which already surfaces it, or point to a block explorer for the SMA address on each chain — so the user sees what's there before deciding how much to add. Frame this as putting the agent to work, not a warning: each required chain funded, the tick loop's balance precondition passes and it acts within the mandate; unfunded, it runs cleanly and skips every tick, logging `balance <n> < min <n>` (see the skeleton's precondition check above) — expected, not a bug, and now the user knows why if they see it.
