...
- **`sailor mandate simulate` only exercises single-call `evaluate()`.** It cannot exercise `evaluateBatch()`. Verify a batch permission by calling `evaluateBatch(calls, ctx)` directly (e.g. `cast call`) with the pass/fail call sequences you expect, before registering it.
