...
**Deterministic address (salt):** every SMA deployment uses a CREATE2 salt. The CLI defaults to salt `0`, giving you a predictable address you can verify before spending gas: `sailor account predict --owner <your-wallet> --manager <agent-wallet>`. The kernel binds the salt to your owner wallet, agent (manager) wallet, and fee policy, so you need the agent wallet created first (the prediction depends on it). The salt is saved in `.sail/account.json` automatically. Note: addresses differ across chains today (Base, Arbitrum, Unichain) because the kernel salt mixes in the chain-specific fee policy and the Safe initializer encodes chain-specific contract addresses; true cross-chain same-address requires a future protocol update (deterministic kernel + safeModuleEnabler + fee policy deployment).
